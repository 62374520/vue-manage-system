<template>
    <div>
                    ...
                    <template #default="{ row, column, $index }" v-if="!item.type">
                        <slot :name="item.prop" :rows="row" :index="$index">
                            <template v-if="item.prop == 'operator'">
                                <el-link type="primary" style="margin-right: 30px;">查看攻击检测结果</el-link>
                                <el-link type="primary" style="margin-right: 30px;">再次执行检测任务</el-link>
        ...
        <el-pagination v-if="hasPagination" :current-page="currentPage" :page-size="10" :background="true"
            :layout="layout" :total="3" @current-change="handleCurrentChange" />
    </div>
</template>
